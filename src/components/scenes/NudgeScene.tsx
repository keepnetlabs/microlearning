...
export function NudgeScene() {
  ...
  // Simulated user data - this would come from props in a real app
  const completionData = {
    ...
    achievements: [
      { id: 'quiz-master', name: 'Quiz Ustası', icon: Award, color: 'text-yellow-600' },
      { id: 'fast-learner', name: 'Hızlı Öğrenen', icon: TrendingUp, color: 'text-green-600' },
      { id: 'security-champion', name: 'Güvenlik Şampiyonu', icon: Shield, color: 'text-blue-600' }
    ]
  };
  ...
  return (
    <div className="flex flex-col items-center justify-start h-full px-4 py-2 sm:px-6 overflow-y-auto relative">
      ...
      >
        ...
        <div className="relative mb-4">
          ...
          >
            ...
            >
              <CheckCircle size={28} className="text-white relative z-10 sm:w-8 sm:h-8" strokeWidth={2.5} />
            </motion.div>
        ...
        <motion.h1
          ...
          className="text-lg sm:text-xl md:text-2xl text-emerald-600 dark:text-emerald-300 mb-2 font-semibold"
        >
          <motion.span
            initial={{ width: 0 }}
            animate={{ width: "auto" }}
        ...
        >
          Siber güvenlik bilgilerinizi başarıyla güncellediniz
        </motion.p>
        ...
        >
          {[
            { icon: Award, value: completionData.totalPoints, label: "Puan", gradient: "from-blue-100 to-indigo-100", darkGradient: "from-blue-900/60 to-indigo-900/60", color: "text-blue-600 dark:text-blue-400" },
            { icon: Clock, value: completionData.timeSpent, label: "Süre", gradient: "from-green-100 to-emerald-100", darkGradient: "from-green-900/60 to-emerald-900/60", color: "text-green-600 dark:text-green-400" },
            { icon: Target, value: "100%", label: "Tamamlama", gradient: "from-purple-100 to-pink-100", darkGradient: "from-purple-900/60 to-pink-900/60", color: "text-purple-600 dark:text-purple-400" }
          ].map((stat, index) => (
            <motion.div 
              key={index}
            ...
            >
              <motion.div 
                className={`flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-r ${stat.gradient} dark:${stat.darkGradient} rounded-xl mb-2 relative overflow-hidden`}
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
        ...
        <div className="flex justify-center">
          <motion.button
            ...
            className="relative flex items-center space-x-2 px-4 py-2 sm:px-6 sm:py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl hover:from-blue-600 hover:to-indigo-700 transition-all shadow-lg hover:shadow-xl disabled:opacity-70 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-blue-500/30 overflow-hidden"
        >
          ...
          <Download size={16} className="sm:w-5 sm:h-5 relative z-10" />
          <span className="text-sm sm:text-base font-medium relative z-10">
            {showCertificate ? 'İndiriliyor...' : 'Sertifikayı İndir'}
          </span>
          ...
          {completionData.achievements.map((achievement, index) => {
            ...
            return (
              <motion.div
                ...
                className="relative flex flex-col items-center p-3 bg-white/60 dark:bg-gray-800/80 backdrop-blur-xl border border-white/40 dark:border-gray-600/60 rounded-xl shadow-sm overflow-hidden group"
              >
                {/* Achievement glow */}
                <motion.div
                ...
                >
                  <Icon size={14} className={`${achievement.color} ${achievement.color.includes('yellow') ? 'dark:text-yellow-400' : achievement.color.includes('green') ? 'dark:text-green-400' : 'dark:text-blue-400'} relative z-10`} />
                </motion.div>
                <span className="text-xs text-center text-gray-700 dark:text-gray-200 leading-tight relative z-10">
                  {achievement.name}
                </span>
              </motion.div>
            );
                ...
                <div className="relative z-10">
                  <div className="flex items-start justify-between mb-1">
                    <div className="flex items-center">
                      ...
                      >
                        <Icon size={14} className={`${action.iconColor} ${action.iconColor.includes('red') ? 'dark:text-red-400' : action.iconColor.includes('amber') ? 'dark:text-amber-400' : 'dark:text-blue-400'} relative z-10`} />
                      </motion.div>
                      <div>
        ...
        <div className="space-y-2">
          {resources.map((resource, index) => (
            <motion.a
              ...
              href={resource.url}
              className="relative flex items-center justify-between p-3 bg-white/50 dark:bg-gray-800/60 backdrop-blur-xl border border-white/30 dark:border-gray-600/50 rounded-lg hover:bg-white/70 dark:hover:bg-gray-700/70 hover:border-white/50 dark:hover:border-gray-600/70 transition-all group overflow-hidden"
            >
              ...
              <div className="flex items-center space-x-3 relative z-10">
                ...
                <div>
                  <div className="text-xs font-medium text-gray-800 dark:text-white group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors">
                    {resource.title}
                  </div>
      ...
      >
        <motion.div
          className="relative p-3 bg-gradient-to-r from-emerald-50/80 to-green-50/60 dark:from-emerald-900/40 dark:to-green-900/30 backdrop-blur-xl border border-emerald-200/60 dark:border-emerald-600/50 rounded-xl overflow-hidden"
        ...
        >
          ...
          <div className="relative z-10">
            <h3 className="text-sm font-semibold text-emerald-800 dark:text-emerald-200 mb-2 flex items-center justify-center">
              <Shield size={16} className="mr-2" />
              Siber Güvenlik Yolculuğunuz Devam Ediyor
            </h3>
            <p className="text-xs text-emerald-700 dark:text-emerald-300 leading-relaxed">
              Bu eğitimi tamamlayarak organizasyonunuzun güvenliğine önemli bir katkı sağladınız. 
              Öğrendiklerinizi uygulamaya devam edin ve ekibinizi de bilinçlendirin.
            </p>
          </div>
        </motion.div>
      </motion.div>
